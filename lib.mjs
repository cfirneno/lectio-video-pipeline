...
export const IMAGE_MODELS = {
  ...
  'nano-banana-pro': { edit: 'fal-ai/nano-banana-pro/edit', text: 'fal-ai/nano-banana-pro', price: 0.15,
    input: (prompt, refs) => ({ prompt, ...(refs.length ? { image_urls: refs } : {}), aspect_ratio: '16:9', resolution: '1K', output_format: 'jpeg' }) },
...
export async function continuityCheck(refs, file, entities, geography = '', transform = {}) {
  ...
  const content = [{ type: 'text', text: `You are a film continuity supervisor. The first ${refs.length} image(s) are approved reference PICTURES. The LAST image is a newly generated shot that must show the same things.\n${legend}\n\nJudge ONLY by comparing the last image with the reference pictures. Do not judge against any written description. For each named entity that is clearly visible in the last image, say whether it is recognisably the SAME one as in its reference picture: same overall shape and proportions, same construction, same materials and colours, same face and hair, same clothing, same vehicle body and armor, same weapons. A different camera angle, distance, crop, lighting, weather, time of day or motion blur is fine. Judge at the scale shown: in a wide view only an entity's overall silhouette, height, proportions and top can be compared, so do not flag stair, bracing, railing or gun details that occupy only a few pixels; in a close view compare the details. If an entity is too small or too blurred to compare at all, say nothing about it. Flag only clear contradictions you can point to in the pixels, such as: a tower with a visibly different structure or top; a truck without its steel front; a man with a different face or beard; a hand-held rifle where the reference has a mounted machine gun.${Object.keys(transform).length ? `\n\nINTENDED CHANGES: in this shot the following are deliberately shown altered, and their alteration must NOT be flagged; check only that what remains of them is consistent with the reference (same materials, colours, construction): ${Object.entries(transform).map(([k, v]) => `${k} - ${v}`).join('; ')}.` : ''}${geography ? `\n\nAlso enforce this fixed geography of the location, and flag a clear violation as entity "GEOGRAPHY": ${geography}` : ''}\nAnswer ONLY with JSON: {"ok": true|false, "problems": [{"entity": NAME, "issue": "one short sentence"}]}` }];
...
export async function makeImage({ modelName, file, key, prompt, refs = [], check = [], geography = '', transform = {}, attempts = 3, charge, log: L, warn: W }) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      verdict = refs.length && check.length ? await continuityCheck(refs, file, check, geography, transform) : null;
